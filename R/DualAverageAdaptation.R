...
DualAveragingAdaptation <- R6Class("DualAveragingAdaptation",
                                   public = list(
                                     ...
                                     adapt_step = function(accept_prob) {
                                       ...
                                       if (!is.infinite(ll) & !is.infinite(self$min_ll)) {
                                         # Update h_bar for logit_tau.
                                         self$h_bar_logit_tau <- (1 - eta_logit_tau) * self$h_bar_logit_tau + eta_logit_tau * (0-ll)
                                       ...
                                       #self$h_bar_logit_tau_2 <- (1 - eta_logit_tau_2) * self$h_bar_logit_tau_2 + eta_logit_tau_2 * (self$target_accept_prob - accept_prob)
                                     ...
                                     get_tau_2 = function() {
                                       self$tau[1]  # tau_2 is already transformed to be in [0, 1]
                                     }
                                   ),
                                   private = list(
                                   )
